# takend from cs236781 course at the Technion
...
class Trainer(abc.ABC):
    ...
    def fit(self, dl_train: DataLoader, dl_test: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            train_loss.append(sum(epoch_res.losses)/len(epoch_res.losses))
            train_acc.append(epoch_res.accuracy)
            epoch_res = self.test_epoch(dl_test, **kw, verbose=verbose)
            test_loss.append(sum(epoch_res.losses)/len(epoch_res.losses))
            test_acc.append(epoch_res.accuracy)
            if checkpoints is not None:
                if best_acc is None or epoch_res.accuracy > best_acc:
                    torch.save(self.model, checkpoints)
            if early_stopping is not None:
                if best_acc is None or epoch_res.accuracy > best_acc:
                    best_acc = epoch_res.accuracy
                    epochs_without_improvement = 0
                else:
...
class BlocksTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        num_correct = sum(y_out[i].argmax(0) == y[i] for i in range(len(y))).item()
